    ...
    module Uptime
      class << self
        ...
        def generate_graph(options = {})
          ...
          graph_type = (!options[:type].blank?) ? options[:type] : Obscured.c('metrics.types').select {|e| e['type'] == :uptime}.first
          ...
          RRD.graph("#{path_graph}#{graph_name}",
            ...
            using_calculated_data 'pin', :calc => 'uin,8000,/,100,*,1,/'
            using_calculated_data 'umin', :calc => 'min,UN,0,min,IF'
            using_calculated_data 'pmin', :calc => 'umin,8000,/,100,*,1,/'
            using_calculated_data 'uout', :calc => 'out,UN,0,out,IF'
            using_calculated_data 'pout', :calc => 'uout,8000,/,100,*,1,/'
            ...
            print_value 'uin:LAST', :format => 'Cur\: %9.0lf %sd'
            print_value 'uin:AVERAGE', :format => 'Avg\: %9.0lf %sd'
            print_value 'umin:MAX', :format => 'Max\: %9.0lf %sd'
          end
        end
      end
    end
