    ...
    module Memory
      class << self
        ...
        def generate_graph(options = {})
          ...
          graph_type = (!options[:type].blank?) ? options[:type] : Obscured.c('metrics.types').select {|e| e['type'] == :memory}.first
          ...
          RRD.graph("#{path_graph}#{graph_name}",
            ...
            using_calculated_data 'pin', :calc => "uin,#{options[:metric]['max']},/,100,*,1,/"
            using_calculated_data 'umin', :calc => 'min,UN,0,min,IF'
            using_calculated_data 'pmin', :calc => "umin,#{options[:metric]['max']},/,100,*,1,/"
            ...
            print_value 'uin:LAST', :format => 'Cur\: %9.0lf %sB'
            print_value 'uin:AVERAGE', :format => 'Avg\: %9.0lf %sB'
            print_value 'umin:MAX', :format => 'Max\: %9.0lf %sB'

            draw_line :data => 'uout', :color => '#0000ff', :label => 'Committed\t'
            print_value 'uout:LAST', :format => 'Cur\: %9.0lf %sB'
            print_value 'uout:AVERAGE', :format => 'Avg\: %9.0lf %sB'
            print_value 'umout:MAX', :format => 'Max\: %9.0lf %sB'
          end
        end
      end
    end
