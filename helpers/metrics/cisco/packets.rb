      ...
      module Packets
        class << self
          ...
          def generate_graph(options = {})
            ...
            graph_type = (!options[:type].blank?) ? options[:type] : Obscured.c('metrics.types').select {|e| e['type'] == :packets}.first
            ...
            RRD.graph("#{path_graph}#{graph_name}",
              ...
              draw_area :data => 'nout', :color => '#ffe000', :label => 'Outbound\t'
              ...
              draw_area :data => 'in', :color => '#66cc66', :label => 'Inbound\t'
