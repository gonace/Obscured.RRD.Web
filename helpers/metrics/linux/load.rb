      ...
      module Load
        class << self
          ...
          def generate_graph(options = {})
            ...
            graph_type = (!options[:type].blank?) ? options[:type] : Obscured.c('metrics.types').select {|e| e['type'] == :load}.first
            ...
            RRD.graph("#{path_graph}#{graph_name}",
              ...
              draw_area :data => 'uin', :color => '#ff584d', :label => '5 Min \t'
              ...
              draw_area :data => 'uout', :color => '#ffB533', :label => '15 Min\t'
